...
export function validaLogin(input) {
  ...
  if (input.validity.valid) {
    input.parentElement.classList.remove("input-container--invalid-inicio-sesion"); //esta clase está en una libreria externa
    input.parentElement.querySelector(".input-message-error-inicio-sesion").innerHTML = ""; //si me salto el campo NO presentará este error rojo
  } else {
    input.parentElement.classList.add("input-container--invalid-inicio-sesion");
    input.parentElement.querySelector(".input-message-error-inicio-sesion").innerHTML =
      mostrarMensajeDeError(tipoDeInput, input); //si me salto el campo presentará este error rojo
  }
}

//T_E_X_T_A_R_E_A
export function validaAreaDeTextoLogin(textarea) {
  let valorDeAreaDeTexto  = validarEspaciosEnBlancoParaTextArea(textarea);
  if (valorDeAreaDeTexto === "") { //si la variable esta vacía
    textarea.parentElement.classList.remove("input-container--invalid-inicio-sesion"); //quitar esta clase al elemento principal
    textarea.parentElement.querySelector(".input-message-error-inicio-sesion").innerHTML = ""; //dejar vacío el mensaje del span
  }else{
    textarea.parentElement.classList.add("input-container--invalid-inicio-sesion");
    textarea.parentElement.querySelector(".input-message-error-inicio-sesion").innerHTML = "Error, no todos los carácteres deben ser espacios en blanco"; //si me salto el campo presentará este error rojo
  }
   //el usuario ya salió del campo que estaba rellenando...
}
...
const validadores = {
  correo: (input) => validarEspaciosEnBlanco(input), //la función validarNacimiento cargará UN VALOR DEL INPUT EN ESPECÍFICO que vive dentro del objeto Validadores
  clave: (input) => validarEspaciosEnBlanco(input), //la función validarNacimiento cargará UN VALOR DEL INPUT EN ESPECÍFICO que vive dentro del objeto Validadores
};
